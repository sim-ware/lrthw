...
#| # (octothorpe) - used as a "comment" character; anything that follows is not
#|                  seem by the interpreter
#| #{variable} - used for string interpolation, substitiutes the vaue for
#|               variable in a statement
#| $ - prepends a variable from the command line ($0 is the actual name of the
#|     script run)
...
#| < - less-than
#| <<TEXT  - start code for a block of text. TEXT on its own line ends it.
#| <= - less-than-equal
#| > - greater-than
#| >= - greater-than-equal
#| ARGV - the argument value (constant), holds arguments entered on the command
#|        line
...
#| STDIN - used with methods when ARGV is used as well, to differentiate between
#|         the two
#| [ ] - used to seprate arguments from puts and print statements
#| \\ - escapes a backslash
#| \n - newline character
#| \t - inserts a tab
#| chomp - used typically with gets. Removes the newline character froma line of
#|         text
...
#| puts - used to "put" a "string" of characters on the screen along with a
#|        newline
